...
// To track the no. of prefix sums of[i-1], use an unordered_map, as in practical cases, it's TC is O(1).
// Create, unordered_map <int,int>, where 1st int is the "prefix sum" & 2nd int is for it's "frequency".
...
int subarraySum(vector<int>& arr, int k) {
    ...
    vector<int> prefixSum(n,0); // to store all prefix sums of the arr // size of this vector is n and initialised with 0
    ...
    for(int j=0; j<n; j++) {
        if(prefixSum[j]==k) count++; //means we have found the req subarray in the beginning itself

        int val = prefixSum[j] - k; // val --> prefixSum[i-1]
