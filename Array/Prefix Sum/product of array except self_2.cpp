...
vector<int> prodExceptSelf(vector<int>& nums) {
    ...
    for(int i=1; i<n; i++) { // 0th index doesn't have any prefix, so starting loop from 1st idx
    ...
    for(int i = n-2; i >= 0; i--) { // suffix of last index (n-1) is 1 (since we had initialised it with 1)
        suffix[i] = suffix[i+1] * nums[i+1]; // suffix of i'th index is the suffix of elements after it multiplied by the num after it
    }
