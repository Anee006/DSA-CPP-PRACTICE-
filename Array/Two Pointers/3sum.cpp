...
// First sort the nums array. After sorting given array we'll have: [-4,-1,-1,0,1,2].
// Let i = -4 (first val in triplet). Since, we've got i, now we need to find j and k in the remaining sorted array.
...
vector<vector<int>> threeSum(vector<int>& nums) {
    ...
    for(int i=0; i<n; i++) {
        ...
        int j = i+1; // left ptr // starting from i+1, as we i!=j!=k
        int k = n-1; // right ptr // our last value in the sorted vector becomes k
...
// TC = O(n^2) and SC = O(k), k --> no. of triplets stored