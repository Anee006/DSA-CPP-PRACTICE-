...
// such that: 0 <= a, b, c, d < n where, a, b, c, and d are distinct.
...
vector<vector<int>> fourSum(vector<int>& nums, int target) {
    ...
    for(int i=0; i<n; i++) { // to pick a unique i value each time

        if(i > 0 && nums[i] == nums[i-1]) continue; // means if we have two same i values, then we'll skip i till we get a unique val
        ...
        for(int j = i+1; j<n;) { //to select j //removed j++ from here and wrote it at end of this loop
            ...
            while(k < l) {
                ...
                else { // found our ans, so add it in the ans vector
                    ans.push_back({nums[i], nums[j], nums[k], nums[l]});
                    k++;
                    l--;

                    while(k<l && nums[k] == nums[k-1]) k++; // to avoid duplicates and get unique val       
                }
            }
            j++; //j++ of the inner loop

            while(j<n && nums[j] == nums[j-1]) j++; // if j is equal to its prev val then, do j++ till we get unique val
        }
    }
    return ans;
}
...
// TC = O(nlogn + n^3), SC = O(uniqueGroups)