...
// 2. if count=1, simply replace the ch in the original vector. If count>1, then it has to be stored as: "ch count". so, we need to 
// convert "count" to string first using "to_string(count)", calculate it's individual digits and then store it in the given vector.
// 3. Create "idx" to track the original index where we need to make the changes. idx helps in adding new char/str into the same vector.
// 4. At the end of loop do i--, so that due to i-- we can go to prev idx and due to i++ of the for loop we can come back at the 
// same idx, and start counting for that idx. agar i-- nhi likhne ka mann kare toh for loop mai se i++ hata do.
