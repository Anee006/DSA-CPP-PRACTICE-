...
void sortArray(vector<int>& arr) {
    ...
    for(int i=0; i<count_0; i++) arr[idx++] = 0;     // overriding with 0

    
    for(int i=0; i<count_1; i++) arr[idx++] = 1;     // overriding with 1

    
    for(int i=0; i<count_2; i++) arr[idx++] = 2;     // overriding with 2
}
