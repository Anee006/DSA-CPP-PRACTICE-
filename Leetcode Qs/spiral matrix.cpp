...
vector<int> spiralMatrix(vector<vector<int>>& matrix) {
    ...
    //The reason for using && is that, which ever of "srow<=erow" or "scol<=ecol" is smaller, will be the controlling factor
    while(srow <= erow && scol <= ecol) {
        ...
        for(int j = ecol-1; j >= scol; j--) {
            // if srow and erow are equal, then in case of odd sized matrix, to avoid duplicates, we break here
            if(srow == erow) break; //the el would have already been printed by the top boundary so, can break the bottom bdry here
        ...
        for(int i = erow-1; i >= srow+1; i--)  {
            // if scol and ecol are equal, then in case of odd sized matrix, to avoid duplicates, we break here.
            if(scol == ecol) break; // the el would have already been printed by the right boundary so, can break the left bdry here
