...
bool isPossible(vector<int>& arr, int n, int m, int MaxAllowedTime) { // here "MaxAllowedTime" is the "mid" value only
    ...
    for(int i=0; i<n;i++) {
        if(arr[i] > MaxAllowedTime) return -1; // edge case

        if(time + arr[i] <= MaxAllowedTime) time += arr[i]; // means can add more time to that painter

        else {
            painter++; // means a new painter is req
            time = arr[i];
        }
    }
...
int minTimeToPaint(vector<int>& arr, int n, int m) {
    ...
    int st = maxValue;
    int end = sum;
    int ans = -1;  // for storing the min time req

    while(st <= end) {
        int mid = st+(end-st)/2;

        if(m > n) return -1; // if no. of painters are more and no. of boards are less, so not possible to calc ans.

        // checking if mid is a valid ans
