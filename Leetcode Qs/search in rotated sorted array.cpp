...
// There is an integer array nums sorted in ascending order (with distinct values). Given the array nums after the possible rotation
...
int search(vector<int>& arr, int target) {
    ...
    while(start <= end) {
        ...
        if(arr[mid] == target) return mid; // returns the req index
        
        if(arr[start] <= arr[mid]) { // for checking if left half is sorted

            // checking if target lies b/w start and mid // if this is true, check in left half
            if(arr[start] <= target && target <= arr[mid]) end = mid-1;

            else start = mid+1; // for checking in right half
        }

        else {  // for checking if right half is sorted
            // for checking if target lies b/w mid and end // if this is true, check in right half
            if(arr[mid] <= target && target <= arr[end]) start = mid+1;
            
            else end = mid-1; // for checking in left half
        }
    }
    return -1;
}
