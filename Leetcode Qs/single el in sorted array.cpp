...
int singleElement(vector<int>& arr) {
    ...
    while(st <= end) {
        ...
        if(mid == 0 && arr[0] != arr[1]) return arr[mid]; // single el occurs at first idx
           
        if(mid == n-1 && arr[n-1] != arr[n-2]) return arr[mid]; // single el occurs at last idx 

        if(arr[mid-1]!= arr[mid] && arr[mid]!= arr[mid+1]) return arr[mid]; // when mid = single el

        // if there are even number of el on both sides of mid
        if(mid %2 == 0) {
            if(arr[mid-1] == arr[mid]) end = mid-1; // answer exists in left side

            else st = mid+1; // answer exists in right side
        }
        
        // if there are odd number of el on both sides of mid
        else {
            if(arr[mid-1] == arr[mid]) st = mid+1; // answer exists in right side
                
            else end = mid-1; // answer exists in left side
        }
    }
    return -1;
}
