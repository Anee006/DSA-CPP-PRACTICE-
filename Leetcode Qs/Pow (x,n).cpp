...
double pow(double x, int n) {
    // Corner cases:
    if(n == 0) return 1.0; // anything raised to power 0 is 1

    if(x == 0) return 0.0; // 0 raised to any power is 0

    if(x == 1) return 1.0; // 1 raised to any power is also 1

    if(x == -1 && n % 2 == 0) return 1.0; // if our base is -1 and power is even then ans is 1

    if(x == -1 && n % 2 == 1) return -1.0; // if our base is -1 and power is odd then ans is -1

    //main logic:
    long binaryForm = n; 
    if(n < 0) { // if negative powers, make it +ve, as in binary exponentiation we need only +ve powers
        x = 1/x;
    ...
    while(binaryForm > 0) {
        ...
        if(binaryForm % 2 == 1) ans *= x; // to check if last digit of the binary form is 1    
        x *= x; // taking x^2 here
