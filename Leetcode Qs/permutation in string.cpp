// Leetcode 567
// Using "Sliding Window" approach: means we try to search the ans by taking a window, which we keep on updating
...
bool checkInclusion(string s1, string s2) {
    ...
    for(int i = 0; i < s2.length(); i++) {
        ...
        while(windowIdx < windowSize && idx < s2.length()) { // window in this case should be of size 2 only and not more than that
            windowFreq[s2[idx] -'a']++; // logic to store this is similar to that of storing char of s1 in frequency array
            windowIdx++;
            idx++;
        }

        // if freq of both is same means that the char occurs same amt of times in both hence, true
        if(isFreqSame(frequency, windowFreq)) return true; 
    }
    return false;
}
