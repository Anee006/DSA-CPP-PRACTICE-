...
// if additional nodes exist in right subtree of the node,then starting from the right child store the left-most nodes in the rst, in stack.
...
public:
    ...
    int next() {
        ...
        storeLeftNodes(ans->right); // to store left-most nodes of current node starting from root, which may exist in its right subtree 
