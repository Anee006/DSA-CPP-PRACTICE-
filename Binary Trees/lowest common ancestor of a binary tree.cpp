...
Node* LCA(Node* root, Node* p, Node* q)
{
    ...
    if (root->data == p->data || root->data == q->data) return root; // if have found the nodes p and q, then assume, that node is LCA

    //vtry to find p and q nodes in the left and right subtree
    Node* leftLCA = LCA(root->left, p , q);
    Node* rightLCA = LCA(root->right, p , q);
    
    if(leftLCA && rightLCA) return root; // if both leftLCA and rightLCA are valid it means that root is the LCA
    
    else if (leftLCA != NULL) return leftLCA; // it means only leftLCA is valid and rightLCA is NULL
    
    else return rightLCA; // it means only rightLCA is valid and leftLCA is NULL
}
