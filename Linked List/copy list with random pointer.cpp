...
Node* copyRandomList(Node* head) {
    if(head == NULL) return NULL; // means linked list is empty
...
void printList(Node* head) {
    ...
    while (temp != NULL) {
        ...
        if (temp->random) cout << ", Random points to: " << temp->random->data; // if random connection exists

        else cout << ", Random points to: NULL\n"; // if no random connection
