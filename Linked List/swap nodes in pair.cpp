...
Node* swapNodes(Node* head) {
    // base case:
    if(head == NULL || head->next == NULL) return head;  // if no node exists or one node exists, in either case we will return head
    ...
    while(first != NULL && second != NULL) { // both first and second should be valid nodes for it to be swapped
        ...
        if(prev != NULL) prev->next = second; // to make the connection of prev and second node after swapping first and second
        ...
        else second = NULL; // means if third points to NULL, we will point the second also to NULL, to stop swapping
    }
    return head;
}
