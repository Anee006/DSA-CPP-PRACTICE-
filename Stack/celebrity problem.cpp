...
int getCelebrity(vector<vector<int>>& arr) {
    ...
    for(int i=0; i<n; i++) s.push(i); // to push all index values into the stack
    
    while(s.size() > 1) { // min two elements should be there in stack for this loop to run
        int i = s.top(); // i'th value
        s.pop();

        int j = s.top(); // j'th value
        s.pop();

        // deciding which idx to eliminate
        if(arr[i][j] == 0) s.push(i); // to eliminate j, we are pushing i
        
        else s.push(j); // to eliminate i, we are pushing j 
    }
