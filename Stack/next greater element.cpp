...
int main() {
    ...
    for(int i = arr.size() - 1; i >= 0; i--) {
        ...
        while(s.size() > 0 && s.top() <= arr[i]) s.pop(); // the el should be strictly greater than the current el. Hence we wrote "=" as well
    
        if(s.empty()) ans[i] = -1; // the next greater el will be -1 whenever stack becomes empty 

        else ans[i] = s.top(); // else, the next greater el will be the el at the top of the stack
