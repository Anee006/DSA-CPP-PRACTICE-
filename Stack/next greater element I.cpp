...
vector<int> nextGreaterElement(vector<int>& nums1, vector<int>& nums2) {
    ...
    for(int i = nums2.size()-1; i>=0; i--) {
        ...
        else m[nums2[i]] = s.top(); // s.top() is the next greater el
    ...
    for(int i = 0; i < nums1.size(); i++) ans.push_back(m[nums1[i]]); // m[nums1[i]] gives the val of next greater el for the current el of nums1
