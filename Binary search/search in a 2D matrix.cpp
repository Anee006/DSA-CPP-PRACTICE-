...
// Each row is sorted in non-decreasing (asc) order.
// The first integer of each row is greater than the last integer of the previous row.
// Given an integer target, return true if target is in matrix or false otherwise.

// 3x4 matrix: {{1,3,5,7}, {10,11,16,20}, {23,30,34,60}} and target = 34.
// Here mxn = 3x4 (is the size of matrix)

// Elements inside matrix are in sorted manner (as the value only keeps on increasing)
...
bool searchInRow(vector<vector<int>>& matrix, int target, int row) {
    ...
    while(st <= end) {
        ...
        if(target == matrix[row][mid]) return true; // "row" is the row which we had found using searchMatrix fn and "mid" is the col
        
        else if(target > matrix[row][mid]) st = mid+1; // search in right side
        
        else end = mid-1; // search in left side
    }
