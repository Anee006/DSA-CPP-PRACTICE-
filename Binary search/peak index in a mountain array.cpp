...
// We can say, for this part arr[i-1] < arr[i] 
...
int peakIndex(vector<int>& arr) {
    int st = 1, end = arr.size() - 2; // modified the search space, to avoid the edge cases

    while(st <= end) {
        int mid = st + (end-st) / 2;

        if(arr[mid-1] < arr[mid] && arr[mid] > arr[mid+1]) return mid; // checking if mid is the peak idx
    ...
    return -1; // this will never execute since peak idx will always exist (as it is given in Q).
}
