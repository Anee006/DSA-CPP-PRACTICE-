...
// if we are starting from first cell i.e 1 at the top left (i.e, (0,0) cell) and going till the last cell that is 1 bottom right 
// (i.e, (n-1, n-1) cell). What are the number of possible ways in which we can reach the end?

// Conditions: 
// 1. from a cell we can either go one up(U), one down(D), one left(L) or one right(R)
...
// NOTE:
// If the 1st cell in the maze i.e, (0,0) cell or the last cell in the maze i.e, (n-1, n-1) was 0, then no solution would have existed 
// for this Q, bcoz, acc to the conditions, we can't go on a cell with 0.
...
void helper(vector<vector<int>>& maze, int r, int c, string path, vector<string>& ans) {
    ...
    // the first four conditions impose the boundary conditions.
    // the second last condition means we cannot go on cell having zero in it 
    // the last condition imposes that we do not need to visit a cell again
    ...
    maze [r][c] = -1; // means visited the cell --> visited = true // means the cell need not be visited again as it has already been visited.
