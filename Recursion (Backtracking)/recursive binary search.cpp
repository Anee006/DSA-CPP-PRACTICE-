// Binary search algo works on a sorted array. Used to find a target in a sorted array.
...
int recBinarySearch(vector<int>& arr, int target, int st, int end) {
    if(st <= end) {
        ...
        else if(arr[mid] <= target) return recBinarySearch(arr, target, mid+1, end); // search in right half

        else return recBinarySearch(arr, target, st, mid-1); // search in left half
    }
