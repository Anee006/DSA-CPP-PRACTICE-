...
bool isSafe(vector<vector<char>>& board, int row, int col, char digit) {
    ...
    for(int j=0; j<9; j++) {
        if(board[row][j] == digit) return false;  // means digit already exists in that row
    }

    // to check vertically
    for(int i=0; i<9; i++) {
        if(board[i][col] == digit) return false;  // means digit already exists in that row
    }
    ...
    for(int i = srow; i <= srow+2; i++) { // TC of this for loop is: O(9) which is const TC. Bcoz only works on the 3*3 grid.
        for(int j = scol; j <= scol+2; j++) {
            if(board[i][j] == digit) return false; // means dig already exists in the grid
        }
    }
...
bool sudokuSolver(vector<vector<char>>& board, int row, int col) {
    if(row == 9) return true; // means the sudoku is solved and our placement of digits is correct
    ...
    for(char digit = '1'; digit <= '9'; digit++) {
        if(isSafe(board, row, col, digit)) { // means it is safe to place digit at that place
            ...
            if(sudokuSolver(board, nextRow, nextCol)) return true; // if this fn returns true, we'll also return true
