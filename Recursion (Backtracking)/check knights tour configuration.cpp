...
bool isValid(vector<vector<int>>& grid, int r, int c, int n, int expectedValue) { // here r = row, c = column and n = size of grid
    ...
    if(expectedValue == n*n - 1) return true; // total moves we can make = n*n - 1. So if expVal == n*n-1 means we have checked all values.
    ...
    return ans1 || ans2 || ans3 || ans4 || ans5 || ans6 || ans7 || ans8; // if even one of the moves return true we will also return true or vice versa
}
