...
bool isCycleDFS(int src, vector<bool>& vis, vector<bool>& recPath, vector<vector<int>>& prerequisites) {
    ...
    for(int i = 0; i < prerequisites.size(); i++) { // loop on all "edges" of the graph
        ...
        if(u == src) { // means "v" will be the neighbor of "u"
            ...
            else if(recPath[v]) return true; // if neighbor is already visited and is also part of the rec path
        }
    }
...
bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
    ...
    for(int i = 0; i < numCourses; i++) {
        if(!vis[i]) {
            if(isCycleDFS(i, vis, recPath, prerequisites)) return false; // means cycle exists --> not a DAG --> return false
        }
    }
    return true;
}
...
int main() {
    ...
    if(canFinish(numCourses, prerequisites)) cout << "Can finish courses\n"; // returns this
    ...
    if(canFinish(numCourses, prerequisites_2)) cout << "Can finish courses\n";
